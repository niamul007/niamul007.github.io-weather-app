...
import DynamicBackgroundController from "./DynamicBackgroundController";
...
export default function WeatherApp() {
  ...
  const fetchData = useCallback(
    async ({ cityName, lat, lon }) => {
      ...
      try {
        ...
        if (currentData.cod && currentData.cod !== 200) {
          throw new Error(
            currentData.message || "City not found or invalid response."
          );
        }
    ...
    [unit]
  ); // Re-create if unit changes (important for initial fetch)
      ...
      if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            console.warn(
              "Geolocation failed or denied. Falling back to London.",
              error
            );
        ...
        console.warn(
          "Geolocation not supported by browser. Falling back to London."
        );
        fetchData({ cityName: "london" });
      }
    };
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage =
                "Location access was denied. Please check your browser's site permissions.";
              break;
            case error.POSITION_UNAVAILABLE:
              errorMessage =
                "Location information is unavailable (network or GPS issue).";
              break;
            case error.TIMEOUT:
  ...
  const generateWeatherSuggestion = async () => {
    ...
    const systemPrompt =
      "You are a friendly and enthusiastic travel planner and activity guide. Your task is to provide a single, concise paragraph with an activity or travel suggestion based on the provided weather information. Focus on the best thing a person can do outdoors or indoors given the current conditions and the 5-day outlook.";
    ...
    try {
      ...
      setSuggestion(
        "Could not connect to the AI service for a suggestion. Please try again."
      );
    } finally {
      setAiLoading(false);
    }
  };
  const currentIconCode = weatherData?.current?.icon || null;

  // --- RENDERING ---
  // --- Code inside your WeatherApp.jsx component ---
  // Remember to extract this variable before the return:

  return (
    // Use a React Fragment to wrap the entire app structure
    <>
      {/* 1. Dynamic Background Controller: Sets the background image on document.body */}
      <DynamicBackgroundController currentIconCode={currentIconCode} />

      {/* 🛑 NEW: Background Darkening Overlay 🛑 */}
      {/* Sits over the background image but beneath the content (z-10) */}
      <div
        className="fixed inset-0 z-10"
        style={{
          // Adjust the opacity (0.45) for desired darkness. Range 0.3 to 0.6 is typical.
          backgroundColor: "rgba(0, 0, 0, 0.45)",
          pointerEvents: "none", // Ensures clicks pass through to the app below
        }}
      />
      {/* -------------------------------------- */}

      {/* 2. Main Content Wrapper: This div must NOT have a static background and must be relative/z-20 */}
      <div className="relative min-h-screen text-white p-4 sm:p-8 font-sans">
        {/* Inner container needs z-20 to ensure it is above the transition overlay (z-999) and the darkening layer (z-10) */}
        <div className="max-w-4xl mx-auto relative z-20">
          <header className="text-center py-6">
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 flex items-center justify-center">
              <span
                role="img"
                aria-label="sun"
                className="mr-3 text-yellow-300 text-4xl"
              >
              </span>
          ...
          <div className="flex justify-end mb-4">
            <button
              onClick={toggleUnit}
              className="p-2 text-white font-semibold rounded-xl shadow-lg transition duration-200 
          ...
          {weatherData && !loading && !error && (
            <div className="weather-display space-y-8 mt-4">
              <CurrentWeatherCard
              ...
              />
              {/* ForecastList uses your glassmorphism ForecastItem components */}
